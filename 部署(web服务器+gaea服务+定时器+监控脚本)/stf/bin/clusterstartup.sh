...
### ====================================================================== ###
##                                                                          ##
##                       SCH server bootstrap script                        ##
##                                                                          ##
### ====================================================================== ###

source /etc/profile

USAGE="Usage: startup.sh <service-name> <service-name-node>"
SYSTEM_PROPERTY=""
...
OTHER_SCH_CONFIG=""
for((i=2; i<=$#; i++)); do
  OTHER_SCH_CONFIG=$OTHER_SCH_CONFIG" "${!i}
done

#if this service is not shutwodn Please shutdown
SERVICE_DIR=`dirname "$0"`
SERVICE_DIR=`cd "$bin"; pwd`
PID_FILE="$SERVICE_DIR"/../tmp/pid/"$SERVICE_NAME"_"$SERVICE_NAME_NODE"
...
DIR="$SCH_HOME"bin
if [ "$DIR" = "bin" ]; then
  echo "no SCH_HOME path"
  DIR=`dirname "$0"`
  DIR=`cd "$bin"; pwd`
fi
PROGNAME=`basename $0`
ROOT_PATH="$DIR"/..
DEPLOY_PATH="$ROOT_PATH"/service
...
# main class
MAIN_CLASS=com.bj58.ecat.emc.scheduler.server.bootstrap.Main


java $JAVA_OPTS -classpath $CLASS_PATH -Duser.dir=$DIR $SYSTEM_PROPERTY $MAIN_CLASS $OTHER_SCH_CONFIG -Dsch.service.name=$SERVICE_NAME -Dsch.service.name.node=$SERVICE_NAME_NODE &

echo pid:$!

echo $! > "$PID_PATH"/"$SERVICE_NAME"_"$SERVICE_NAME_NODE"
